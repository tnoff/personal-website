...
from basketball.management.urls import BASE_URL
...
def _get_team_metadata(soup):
    ...
    for (count, team) in enumerate(teams):
        ...
        try:
            ...
            team = Team.objects.filter(year=team_data['year'], short_name=team_data['short_name']).first()
...
def process_box_score(box_score_url):
    ...
    try:
        ...
        game = Game.objects.filter(date=game_date, away_team=away_team, home_team=home_team).first()
    return game
